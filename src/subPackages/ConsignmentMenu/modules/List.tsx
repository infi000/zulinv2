...
const List = () => {
  const { goodsData, listScroll } = useSelector((state) => state.ConsignmentMenu);
            ...
            formatList.map((item, index) => {
              ...
              return (
                <View className='at-col at-col-6 goods-con' key={id + index}>
                  <View className='goods-con-mid' onClick={() => handleChooseGoods({ id: jgid, issale: issale, title:jgtitle, orderid })}>
                    {issale === '-1' && (
