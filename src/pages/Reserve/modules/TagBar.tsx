...
const TagBar = (props) => {
  ...
  const { allCtypeList,experimentsData } = useSelector((state) => state.reserve);
  ...
  useEffect(() => {
    ...
    dispatch({ type: 'reserve/updateExperimentsDataParams', payload });
    dispatch({ type: 'reserve/getSearchExperiments' });
  }, [tabCurrent]);

  useEffect(() => {
    dispatch({ type: 'reserve/getAllCtype' });
  }, []);
  return (
